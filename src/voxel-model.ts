...
class VoxModel {
    id: string = "";       //  Default: "VOX "
    version: number = 0;   //  Default: 150
    MAIN: VoxChunk;
    byteLength: number = 0;
     ...
     * @param voxBuffer
     */
    constructor(voxBuffer: ArrayBuffer) {
        const self = this;
    ...
    getPalette_strHexRGBA(index: number): string {
        let palette = this._Palette();
        let numColor = palette[index];
        let strColor = numColor.toString(16);
        strColor = strColor.split("").reverse().join(""); //  反转
        return strColor;
    };

    getPalette_Hex(index: number): number {
        let color: string = this.getPalette_strHexRGBA(index);
...
class VoxChunk {
    ...
    constructor(buffer: ArrayBuffer) {
        ...
        try {
            ...
            iter.next();
            self.id = VoxToolkit.buffer2String(
                {buffer: buffer, begin: iter.begin, end: iter.end});

            iter.next();
            self.contentSize = VoxToolkit.buffer2Int(
                {buffer: buffer, begin: iter.begin, end: iter.end});

            iter.next();
            self.childrenSize = VoxToolkit.buffer2Int(
                {buffer: buffer, begin: iter.begin, end: iter.end});
            ...
            if (self.childrenSize > 0) {
                ...
                let childrenBuf = buffer.slice(iter.begin, iter.end);
    ...
    private _Children(childrenBuf: ArrayBuffer): void {
        if (childrenBuf.byteLength <= 0)    return;
...
function voxChunkHandler(chunk: VoxChunk): VoxChunk {
    const ID_LIST = {
        MAIN: "MAIN", PACK: "PACK",
        SIZE: "SIZE", XYZI: "XYZI",
        RGBA: "RGBA", MATT: "MATT"
    ...
    const handlerList = {
        MAIN: function (chunk) {
            ...
            for (let i = 0; i < chunk.children.length; i++) {
                ...
                switch (item.id) {
                    ...
                    case ID_LIST.XYZI:
                        let lastItem = chunk.models[chunk.models.length - 1];
                        lastItem.XYZI = item;
                        break;
        ...
        PACK: function (chunk) {
            chunk.numModels = VoxToolkit.buffer2Int(
                {buffer: chunk.content, begin: 0, end: 4});
        },

        SIZE: function (chunk) {
            chunk.x = VoxToolkit.buffer2Int(
                {buffer: chunk.content, begin: 0, end: 4});
            chunk.y = VoxToolkit.buffer2Int(
                {buffer: chunk.content, begin: 4, end: 8});
            chunk.z = VoxToolkit.buffer2Int(
                {buffer: chunk.content, begin: 8, end: 12});
        ...
        XYZI: function (chunk) {
            let begin = 0, end = 4;
            chunk.numVoxels = VoxToolkit.buffer2Int(
            ...
            let XYZI_value;
            for (let i = 0; i < chunk.numVoxels; i++) {
                begin = end;
                end = begin + 4;
                ...
                XYZI_value = VoxToolkit.buffer2Uint8Arr(
                    {buffer: chunk.content, begin: begin, end: end});
        ...
        MATT: function (chunk) {
            ...
            iter.next();
            chunk.m_ID = VoxToolkit.buffer2Int(
                {buffer: chunk.content, begin: iter.begin, end: iter.end});


            iter.next();
            chunk.m_Type = VoxToolkit.buffer2Int(
                {buffer: chunk.content, begin: iter.begin, end: iter.end});

            iter.next();
            chunk.m_Weight = VoxToolkit.buffer2Float(
                {buffer: chunk.content, begin: iter.begin, end: iter.end});

            iter.next();
            chunk.m_Bits = VoxToolkit.buffer2Int(
                {buffer: chunk.content, begin: iter.begin, end: iter.end});


            chunk.m_Values = [];
            let valueCount = (chunk.contentSize - iter.end) / 4;
            for (let i = 0; i < valueCount; i++) {
                iter.next();
                let item = VoxToolkit.buffer2Float(
                    {buffer: chunk.content, begin: iter.begin, end: iter.end});
...
class VoxToolkit {
    ...
    static littleEndian: boolean = function () {
    ...
    static buffer2String(options: VoxToolkit_Options): string {
        let result = "";
        try {
    ...
    static buffer2Int(options: VoxToolkit_Options): number {
        let result = 0;
        try {
            // let buf = options.buffer.slice(options.begin, options.end);
            let dataView = new DataView(
                options.buffer, options.begin, options.end - options.begin);
    ...
    static buffer2Float(options: VoxToolkit_Options): number {
        let result;
        try {
            let dataView = new DataView(
                options.buffer, options.begin, options.end - options.begin);
    ...
    static buffer2Uint8Arr(options: VoxToolkit_Options): Uint8Array {
